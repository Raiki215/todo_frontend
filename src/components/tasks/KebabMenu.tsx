...
export default function KebabMenu({
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {/* ドロップダウンメニュー（Portalを使わずシンプルに） */}
      {mounted && isOpen && (
        <div className="absolute right-0 z-50 w-40 py-1 bg-white border border-gray-200 rounded-lg shadow-lg top-8">
          <button
            onClick={(e) => {
              e.stopPropagation();
              e.preventDefault();
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              e.preventDefault();
              handleMenuClick(onMoveToTomorrow);
            }}
            className="flex items-center w-full gap-2 px-3 py-2 text-sm text-left text-gray-700 hover:bg-gray-50"
          >
            <svg
